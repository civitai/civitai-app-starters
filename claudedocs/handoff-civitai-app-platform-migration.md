...
- **Next probe:** 🔴 **on moderator approval, load `https://oauth-probe.civit.ai` in a real page slot and READ THE SCREEN.** Expect `token kind = oauth`; `block` is a failed opt-in and `unknown` is a stale host — the app keeps those three apart on purpose, so the reading is not ambiguous. Then: does the host prompt for `buzz:read:self`, does `/api/v1/me` withhold `email` before consent and return it after, and does `tokenScope` move `1` → `65537`. 🔴 **The discriminator is built in** — the app reports the BITMASK, never the returned-field list, because `me.ts` omits `isModerator` for every non-moderator and `email` for any account without one, so an absent field cannot tell consent from circumstance.
...
2. ✅ **DONE 2026-09-25 — the first OAuth opt-in is submitted.** `oauth-probe`, a scratch app (`ZacxDev/civitai-app-oauth-probe` PR #1, `pubreq_01M3CXP8K4K7CTSZ1WGPXBKCM5`, pending moderator review). See the RESOLVED investigation block — no fleet app could be the first opt-in, and two of this doc's own scope claims were false. 🔴 **The remaining work is the PROBE, not the build:** on approval, read the screen at `https://oauth-probe.civit.ai`. Nothing has yet run against a real host.
