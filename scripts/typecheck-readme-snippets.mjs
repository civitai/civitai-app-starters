...
const ENTRYPOINTS = [
  ...
  // Type-only subpath: the orchestrator's generated step templates, re-exported
  // from `@civitai/client` (an OPTIONAL peer). Listed here so README snippets
  // that import from it are resolved against the real declarations rather than
  // shimmed to `any`. Its entry exports no values at all, so it contributes
  // only to the TYPE map below — which is the point: an omitted-import
  // reference to `TypedWorkflowTemplate` still validates against the true type.
