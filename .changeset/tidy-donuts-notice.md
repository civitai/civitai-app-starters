...
`WORKFLOW_STEP_TYPES` told you which `$type`s exist; the body builders took `input: unknown`. This adds the other half — the real per-step input shapes, re-exported from the orchestrator's own generated client (`@civitai/client`) so they track the OpenAPI spec by codegen instead of by hand.

New exports (all types): `WorkflowStepTemplates` (`$type` → template, all 47), `WorkflowStepTemplateFor<T>`, `WorkflowStepInputFor<T>`, `AnyWorkflowStepTemplate` (discriminated union — the base `WorkflowStepTemplate.$type` is a bare `string` and narrows nothing), `TypedWorkflowTemplate` (submit envelope with `steps` narrowed), plus the 47 `*StepTemplate` names and the base `WorkflowStepTemplate` / `WorkflowTemplate`. The 47 `*Input` types are deliberately not re-exported individually — `WorkflowStepInputFor<T>` covers that.
