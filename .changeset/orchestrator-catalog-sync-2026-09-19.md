...
- `WORKFLOW_STEP_TYPES`: add the `imageScanning` step type *(description still a placeholder — see below)*
- `WORKFLOW_STEP_TYPES`: add the `preprocessVideo` step type *(description still a placeholder — see below)*
- `WORKFLOW_STEP_TYPES`: add the `yuE2` step type
- `IMAGE_GEN_ENGINES`: add the `flux1-pro` imageGen engine *(description still a placeholder — see below)*
