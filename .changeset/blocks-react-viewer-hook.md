...
The trust-boundary validator `isValidViewerResult` is wired into `payloadValidatorFor`; it validates `viewer.{id,username,status}` and keeps `buzzBudget` OPTIONAL (an over-strict guard that dropped a valid reply is the class of bug that previously hung a read hook). The `createMockHost` (canned viewer + `viewerError` knob) + `createLiveHost` (forwards to the `blocks.getMyViewer` tRPC mutation) dev harnesses answer the bridge.
