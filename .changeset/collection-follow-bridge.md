...
- 🔴 **The guard that was supposed to cover it did not exist.** The control carried an attempt COUNTER whose comment claimed to close "a settle arriving after the parent moved `followed`" — but it was incremented only inside `toggle()`, so it could not observe the parent at all. **Deleting all three of its lines left the file's suite fully green**, which is how the false claim survived review. It is replaced by the correlation guard, and four cases now fail if that guard is removed.
