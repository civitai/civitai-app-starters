...
| `src/live.ts` | 🔴 **`@civitai/blocks-react/live` — THE REAL BACKEND. `createLiveHost` forwards the postMessage protocol to civitai.com over a pasted dev block token, `blocks.submitWorkflow` included, and a successful generation SPENDS THE TOKEN HOLDER'S REAL BUZZ.** Its own subpath since #334, because it used to sit one autocomplete entry from `createMockHost` inside a module called "testing". A `dev:live` harness is the only legitimate caller. Same ledger rules as above (README § "The `/live` subexport"). |
