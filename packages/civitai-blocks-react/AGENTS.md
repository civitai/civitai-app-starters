...
- 🔴 **A new hook on `src/index.ts` ships a `Use<Hook>` return type, no exceptions (#380).** Declare and export it from the hook's own module, re-export it as a type from `src/index.ts`, annotate the hook's return with the NAME (never an inline literal), and add an `Exact<ReturnType<typeof useX>, UseX>` line to `src/hooks/returnTypeLedger.ts`. Aliases count — `UseBlockResize = void` and `UseBlockTheme = Theme` exist so the rule has nothing to remember. **Three checks, three different failure modes, none subsuming another:** `tests/guards/blocks-react-hook-return-types.test.mjs` asserts the hook SET against a written ledger (so it fails when the set GROWS, not only when a type is deleted) and that the annotation is the name; `returnTypeLedger.ts` fails `tsc` when the name stops describing what the hook returns. A hook that is genuinely internal goes in that guard's `INTERNAL_HOOKS` and stays off `src/index.ts` — `useRequestSequencer` is the only one.
