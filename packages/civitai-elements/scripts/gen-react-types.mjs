 ...
 * `packages/civitai-elements-react/test/react19-custom-elements.browser.test.tsx`
 ...
 *
 * Run: pnpm --filter @civitai/elements gen:react
 */
...
const reactPkg = join(pkgRoot, '..', 'civitai-elements-react');
const outDir = join(reactPkg, 'src', 'generated');

const manifest = JSON.parse(readFileSync(join(pkgRoot, 'custom-elements.json'), 'utf8'));
...
if (elements.length === 0) throw new Error('[gen-react] manifest declares no tagged elements');
...
lines.push('   scripts/gen-react-types.mjs. DO NOT EDIT.');
lines.push('   Regenerate: pnpm --filter @civitai/elements gen:react');
lines.push('   Guarded by test/generated-types-parity.test.ts. */');
lines.push('');
...
console.log(
  `[gen-react] wrote ${join('src', 'generated', 'jsx.ts')} for ${elements.length} elements: ${elements
    .map((e) => e.tagName)
    .join(', ')}`
);
