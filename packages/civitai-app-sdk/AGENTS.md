...
| `schemas/app-block/v1.json` | **THE canonical manifest contract** (JSON Schema draft 2020-12) — a byte-identical vendored copy of https://civitai.com/schemas/app-block/v1.json, enforced by `scripts/check-canonical-schema.sh`. Shipped at the `./schemas/app-block/v1.json` subpath for offline validation. 🔴 It is the AUTHORITY, not a mirror: `defineBlock` enforces every rule it expresses mechanically and nothing more, except the itemised `SCHEMA_DIVERGENCES` table (each entry a server rule the canonical states only in prose, with its reason). `test/blocks/schema-parity.test.ts` runs Ajv over this file and fails if the two verdicts disagree anywhere the table does not name — the drift that produced #330. |
