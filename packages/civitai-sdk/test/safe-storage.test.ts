...
describe('installSafeStorage', () => {
  ...
  it('repairs a revoked Proxy without propagating its TypeError', () => {
    ...
    // reject `import '@civitai/sdk'` and take the whole block down.
...
describe('import-time regression (the shape that broke production)', () => {
  ...
  it('importing the safe-storage module first lets that dependency load', async () => {
    restores.push(breakGlobalStorage());
    vi.resetModules();
  ...
  // 🔴 THE REGRESSION. Everything above tests the module; this tests the WIRING,
  // which is the part that was missing. A block that has finished porting to
  // `@civitai/sdk` imports nothing else — no `@civitai/app-sdk/blocks`, no
  // `@civitai/blocks-react` — so the package root is its only chance to be
  // repaired. Delete the `import './safe-storage/index.js'` from `src/index.ts`
  // and this goes red with the SecurityError; the tests above all stay green.
  it('REGRESSION: importing the package root (@civitai/sdk) is enough on its own', async () => {
    restores.push(breakGlobalStorage());
    vi.resetModules();

    // An app reaches the shim without knowing it exists: the public entry every
    // app and block builds against imports it first.
  ...
  // The behavioural test above passes in vitest whatever `package.json` says —
  // vitest does not tree-shake. A bundler does, and `sideEffects: false` is a
  // standing promise to EVERY bundler that `dist/index.js` can be dropped when
  // its exports are unused, taking the install with it. So pin the manifest
  // value itself, as the whole normalised array rather than a substring: a
  // reworded or reordered allowlist that still names both modules passes, and a
  // regression to `false` — or to an allowlist that forgets either module —
  // fails.
  it('REGRESSION: package.json declares the sideEffects allowlist, never `false`', () => {
    ...
    expect([...(manifest.sideEffects as string[])].sort()).toEqual([
      './dist/index.js',
