...
2. **`server/app.ts`** — add a Hono route. Read session via
   `readSession(c, production)`; return 401 if null. Mirror the shape of
   `app.get('/api/me', ...)`.

3. **`src/lib/api.ts`** — add a typed `fetch` wrapper for the SPA. Mirror
   `getMe()`. Throw on non-2xx so error boundaries surface it.

4. **`src/components/<consumer>.tsx`** — consume the wrapper. Never call
