...
import { estimateCost, getWorkflow, submitGeneration } from '../lib/api';
import {
  DEFAULT_MODEL_AIR,
  extractImageUrls,
...
export function GenerateForm({ initialBalance }: { initialBalance?: number }) {
  ...
  const estimate = useCallback(async () => {
    ...
    setPreviewCost(null);
    try {
      const res = await estimateCost({ prompt });
  ...
  const longPollUntilDone = useCallback(async (id: string) => {
    ...
    try {
      while (Date.now() < deadline) {
        const { snapshot: snap, done } = await getWorkflow(id, SERVER_WAIT_MS, controller.signal);
        setSnapshot(snap);
        if (done) {
          setPhase('done');
          return;
  ...
  const submit = useCallback(async () => {
    ...
    setSnapshot(null);
    try {
      const { workflowId: id } = await submitGeneration({ prompt });
      setWorkflowId(id);
      setPhase('polling');
      longPollUntilDone(id);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'unknown');
      setPhase('error');
    }
    ...
    <section className="flex flex-col gap-4">
      <label className="flex flex-col gap-1 text-sm">
        ...
        <span className="text-xs text-zinc-500">
          ...
          <code className="mx-1 font-mono">server/civitai.ts</code> to change)
        </span>
      </label>
      ...
      {phase === 'done' && (
        <div className="flex flex-col gap-2">
          ...
          {imageUrls.length === 0 && snapshot && (
            <>
              <p className="text-sm text-zinc-500">No images in the response. Workflow snapshot:</p>
