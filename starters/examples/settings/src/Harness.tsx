...
const DEV_INSTANCE_ID = 'bki_dev_settings';
const DEV_BLOCK_ID = 'settings-demo';
...
export function Harness({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const payload: BlockInitPayload = {
      ...
      context,
      settings: {
        // Seeded so the example shows non-empty values. In production the
        // host populates these from the install's stored settings.
        publisherSettings: { watermark_text: 'made on civitai', default_steps: 30 },
        userSettings: { show_advanced: true, preferred_sampler: 'DPM++ 2M Karras' },
      },
